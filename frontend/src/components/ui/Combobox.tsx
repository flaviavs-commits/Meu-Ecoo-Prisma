...
export function Combobox({ label = 'Selecionar', options, value = '', onChange, placeholder = 'Digite para buscar', className = '' }: ComboboxProps) {
  const inputId = useId(); const listaId = useId()
  ...
  useEffect(() => {
    if (!open) return
    function fecharAoClicarFora(event: MouseEvent) { if (!raiz.current?.contains(event.target as Node)) setOpen(false) }
    document.addEventListener('mousedown', fecharAoClicarFora)
    return () => document.removeEventListener('mousedown', fecharAoClicarFora)
  }, [open])
  return <div ref={raiz} className={['relative grid gap-2 text-sm text-texto', className].join(' ')}><label htmlFor={inputId}>{label}</label><input id={inputId} role="combobox" aria-expanded={open} aria-controls={listaId} aria-autocomplete="list" value={query} placeholder={placeholder} onFocus={() => setOpen(true)} onChange={(event) => { setQuery(event.target.value); setOpen(true) }} onKeyDown={handleKeyDown} className="min-h-11 rounded-lg border border-contorno bg-superficie px-3.5 text-sm outline-none transition-colors focus:border-texto focus:ring-2 focus:ring-texto/10" />{open && filtered.length > 0 && <ul id={listaId} role="listbox" className="absolute inset-x-0 top-full z-20 mt-2 max-h-56 overflow-auto rounded-lg border border-contorno bg-superficie p-1 shadow-xl">{filtered.map((option) => <li key={option} role="option" aria-selected={option === query}><button type="button" className="min-h-10 w-full rounded-md px-3 text-left text-sm hover:bg-superficie-alt" onMouseDown={(event) => event.preventDefault()} onClick={() => choose(option)}>{option}</button></li>)}</ul>}</div>
}
