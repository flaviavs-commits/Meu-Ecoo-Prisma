...
[2026-08-03] **Painel de superadmin iniciado por Analizar o front do Prisma**: aberta a etapa E14 para um app Django no mesmo serviço Railway, em `/painel/`, inspirado no painel do Vs. A escolha do usuário foi tratar “tier” como o perfil atual (`ALUNO`, `PROFESSOR`, `DIRETOR`), com troca livre pelo superadmin, motivo obrigatório e auditoria. Primeira fatia implementada: dashboard, usuários, detalhe e troca de perfil; senha, tokens e conversa crua de aluno não aparecem. Validação: `pytest painel_admin/tests/test_painel_superadmin.py -q` retornou `4 passed`; `manage.py check` sem issues em SQLite local. Estado: **em andamento**, faltam os módulos restantes de operação e deploy Railway.
