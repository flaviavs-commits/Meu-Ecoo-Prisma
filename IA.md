...
[2026-08-05] O backend MVP e a integração de autenticação estão implementados localmente: Django/DRF das etapas E01 e E03-E11, SQLite para TDD, frontend publicado anteriormente na Vercel e API no Railway. O login de superadmin agora cria a sessão Django e chega ao painel pelo proxy same-origin; a origem pública da Vercel ainda precisa ser configurada em `DJANGO_CSRF_TRUSTED_ORIGINS` antes das ações POST do painel. A validação remota final continua separada por depender de deploy/configuração operacional.
